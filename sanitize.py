...
为什么需要它（不是洁癖，是成本问题）：
  index.html 是完整云端版，原本带着真实的 CLOUD_ENDPOINT 和 publishable key。
  把它连同文档里的链接一起开源，等于公开一个「谁都能注册的后端入口」——访客的
  每一次写入都记在这个应用所有者（也就是你）的资源点账上。而 GitHub Pages 会把
  仓库根目录整个当站点发布，index.html 就是默认首页，所以仓库里有它，入口就等于公开。

本脚本处理三类文件：
  index.html      endpoint / key 换占位符；ver.json 改相对路径；界面文案不再印出部署域名
  use.html        说明书页的「打开应用」指向演示版，不再指向个人部署
  tests/sync.js   测试用的 origin 从 index.html 里现读，不再把域名写死
...
# ---------- 1. index.html ----------
patch('index.html', '云 endpoint 占位符',
...
patch('use.html', '封面按钮指向演示版',
      ...
      '<a class="cta" href="demo.html">打开演示版</a>',
      regex=True, already='<a class="cta" href="demo.html">')
...
patch('tests/sync.js', '测试 url 用 endpoint',
      ...
      "url: EP + '/',\\1// 与 index.html 的 CLOUD_ENDPOINT 一致，让 CLOUD_ONLINE === true",
