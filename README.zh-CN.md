...
**在线试用**：https://yuxinhou32-ux.github.io/voice-schedule/ （无账号、无云端，打开即用）
**使用说明**：[use.html](use.html)（同一个站点里）

形态是 PWA：用 Safari 打开这个链接，添加到主屏幕，就和装了个 App 一样，不用上架应用商店。

> **公开的是哪个版本**：上面这条链接是**无云版本**——不登录、不向任何服务器发数据，所有操作只存在访客自己的浏览器里，预置了一批虚构示例日程。带云同步的完整版 `app.html` 作为源码公开，但**配置是占位符，也没有任何公开部署**。这是刻意的：一个「谁都能注册」的线上实例，访客的每次写入都记在所有者账上，所以这个应用的线上实例是私有的。想用云同步，把配置指向自己的云项目即可。
...
**前端**：单个 `app.html`，样式和逻辑都内联，没有构建步骤、没有框架、没有 npm 依赖（FullCalendar 本地引入，不走 CDN）。打开就能跑，改一行刷新就生效。
...
> 仓库里的 `CLOUD_ENDPOINT` / `CLOUD_KEY` 是**占位符**，不是可用的部署——提交前由 `sanitize.py` 换掉。顺带一个值得知道的边界：`wbpk_` 前缀的 publishable key 本就是设计成公开的（只用于前端初始化云 SDK、标识应用，安全边界在服务端按账号的行级安全 RLS），**真正不该公开的是「可注册的线上实例链接」**——访客的每次写入都记在那个所有者账上。这条线我替这个仓库划了。
...
```
.
├── index.html            无云演示版 —— 站点首页就是它（由脚本生成，不要手改）
├── app.html              带云同步的完整版（单文件：结构 + 样式 + 逻辑）
├── build_demo.py         从 app.html 构建 index.html（移除云 SDK / 版本检查，注入示例数据）
├── sanitize.py           发布前脱敏（把真实 endpoint / key / 链接换成占位符）
...
> **关于 `index.html`**：它就是生成的「无云演示版」，也是站点首页，**不需要手改**。每次主应用发版后重跑一次 `python build_demo.py` 即可。演示版零后端依赖——脚本自带校验，产物里云 SDK 外链、任何绝对外链、云端密钥、`fetch` 调用，残留全部为 0。`sanitize.py` 每次发版还会再断言一次：站点首页一旦重新出现云 SDK / 云端密钥 / 真实的 origin 判定，直接失败退出。

> **发版顺序**（公开副本不出问题的关键）：从主应用同步新版本（改名成 `app.html`）→ `python sanitize.py` → `python build_demo.py` → `npm test` → 提交。`sanitize.py` 最后会扫描所有被 git 跟踪的文件，找真实域名、密钥、应用 ID 和本机绝对路径，发现任何残留就直接失败退出。

## 本地运行与测试

前端没有构建步骤，起一个静态服务器，打开 `app.html`（完整版）或 `index.html`（演示版）即可：
...
> 云端登录只在页面 origin 与 `CLOUD_ENDPOINT` 一致时开启（代码里有一处来源校验），本地或任何其他域名下只会启用本机存储。
