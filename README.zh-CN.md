...
**演示版**：https://yuxinhou32-ux.github.io/voice-schedule/demo.html （无账号、无云端，打开即用）
**使用说明**：[use.html](use.html)（同一个站点里）

形态是 PWA：用 Safari 打开演示版，添加到主屏幕，就和装了个 App 一样，不用上架应用商店。

> **公开的是哪个版本**：上面这条链接是**无云版本**——不登录、不向任何服务器发数据，所有操作只存在访客自己的浏览器里，预置了一批虚构示例日程。带云同步的完整版 `index.html` 作为源码公开，但**配置是占位符，也没有任何公开部署**。这是刻意的：一个「谁都能注册」的线上实例，访客的每次写入都记在所有者账上，所以这个应用的线上实例是私有的。想用云同步，把配置指向自己的云项目即可。
...
> **关于 `demo.html`**：它是 `build_demo.py` 从 `index.html` 生成的演示版，**不需要手改**。每次主应用发版后重跑一次 `python build_demo.py` 即可。演示版零后端依赖——脚本自带校验，产物里云 SDK 外链、任何绝对外链、云端密钥、`fetch` 调用，残留全部为 0。

> **发版顺序**（公开副本不出问题的关键）：从主应用同步新版本 → `python sanitize.py` → `python build_demo.py` → `npm test` → 提交。`sanitize.py` 最后会扫描所有被 git 跟踪的文件，找真实域名、密钥、应用 ID 和本机绝对路径，发现任何残留就直接失败退出。
